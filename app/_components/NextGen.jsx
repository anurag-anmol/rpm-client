...
function NextGen() {
  ...
  return (
    <section id="Features" className="relative py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-[#0c0c1d] overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-12 sm:mb-16">
          {details.map((item, idx) => (
            <div
              key={idx}
              className="relative p-6 sm:p-8 bg-[#120f20] rounded-lg shadow-md hover:shadow-lg transition-shadow hover:border border-muted-foreground hover:animate-bounce duration-300"
            >
              {/* Icon */}
              <div className="mb-4">
                <img
                  ...
                  loading="lazy"
                  alt={`${item.title} icon`}
                  className="bg-white rounded-lg w-16 h-16 sm:w-[70px] sm:h-[70px]"
                />
              </div>

              {/* Title */}
              <h3 className="font-bold text-xl sm:text-2xl text-white mb-2">{item.title}</h3>

              {/* Description */}
              <p className="text-muted-foreground font-medium text-[16px] sm:text-[18px] mb-4">
              ...
              <a href="/" className="flex flex-col group">
                <div className="flex items-center gap-2">
                  <span className="text-sm sm:text-[14px] font-semibold uppercase tracking-wider text-white">
                    Explore Features
                  </span>
                </div>
                <div className="w-[20%] h-[1px] bg-white mt-1 group-hover:w-32 transition-all duration-300"></div>
              </a>
        ...
        <div className="text-center">
          <a
            href="/"
            className="inline-flex items-center px-6 py-3 sm:px-8 sm:py-4 bg-white text-black rounded-full group overflow-hidden transition-all duration-300 hover:bg-opacity-90"
          >
            <span className="mr-2 group-hover:-translate-y-1 transition-transform duration-300 font-bold text-[18px] sm:text-[20px]">
              contact us
            </span>
            <img
              src="https://cdn.prod.website-files.com/68149cfdae7cc5226d0569ac/6814a7937c6031105cd4ab11_arrow-right.avif"
